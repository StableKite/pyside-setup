...
public:
    ...
    /// How deep this thread is nested in the lazy path. Above one, the thread
    /// is building a type and has come back for it, so it is the one caller
    /// that must be given the early publication. Counted even when the lock
    /// is switched off: the A/B column takes the mutual exclusion away, not
    /// the re-entrancy, and a builder still has to see its own type.
    static int &depth()
    {
...
{
    typeStruct.ready.set();
}
#endif

// May the type be handed out? Under free threading that is the readiness
// flag, and a pointer on its own is not enough; a build with a GIL has no
// flag and never had the problem.
static bool typeIsUsable(const TypeInitStruct &typeStruct)
{
#ifdef Py_GIL_DISABLED
    return typeStruct.ready.isSet();
#else
    return typeStruct.type != nullptr;
#endif
}

LIBSHIBOKEN_API PyTypeObject *get(TypeInitStruct &typeStruct)
{
    // Not "is there a type" but "is the type finished". The generated
    // initialization publishes typeStruct.type before it registers the
    // converter, so a plain pointer check used to hand a half-built type to
    // another thread, which then reached copyToPython() with a null
    // converter. The acquire pairs with the release in setReady().
    if (typeIsUsable(typeStruct))
...
#ifdef Py_GIL_DISABLED
    LazyInitLock lock;
    // Either the type is finished - the builder we waited for had set the flag
    // before it released the lock - or this thread is the builder itself and
    // has re-entered. Only for that one caller is the early publication the
    // right answer; it is what breaks the recursion.
    if (typeIsUsable(typeStruct)
        || (LazyInitLock::depth() > 1 && typeStruct.type != nullptr)) {
        return typeStruct.type;
    }
#endif
