...
PyObject *PySideQmlForeignPrivate::tp_call(PyObject *self, PyObject *args, PyObject * /* kw */)
{
    ...
    // Publish the type and its alias in one registry transaction so another
    // thread cannot observe the foreign type without finding its QML info.
    PySide::Qml::setQmlForeignType(klass, data->type());
...
PyTypeObject *PySideQmlForeign_TypeF(void)
{
    ...
    return Shiboken::TypeInit::publish(type, createPySideQmlForeignType);
#else
